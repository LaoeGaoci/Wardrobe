...
class ApiConfig {
  ...
  static const String baseUrl = String.fromEnvironment(
    'API_BASE_URL',
    defaultValue: 'https://api.laoegaoci.win',
  );
...
class ApiClient {
  ...
  String resolveUrl(String path) {
    ...
    return _buildUri(normalized, null).toString();
  }
  ...
  Future<Map<String, dynamic>> putMultipartFile(
    ...
    if (!await file.exists()) {
      throw const ApiException('待上传的图片不存在');
    }
    ...
    if (length <= 0) {
      throw const ApiException('图片文件为空');
    }

    if (length > maxImageSize) {
      throw const ApiException('图片不能超过 10MB');
    }

    final uploadInfo = _detectImageType(filePath);

    final uri = _buildUri(path, null);

    _debugRequest('PUT', uri);

    final boundary = '----wardrobe-${DateTime.now().microsecondsSinceEpoch}';
    ...
    try {
      ...
      throw const ApiException('无法连接服务器，请检查网络连接后重试');
    } on http.ClientException {
      throw const ApiException('网络请求失败，请检查网络连接后重试');
    }
  }
  ...
  Uri _buildUri(String path, Map<String, String>? queryParameters) {
    final configuredBase = ApiConfig.baseUrl.trim();
  ...
  Map<String, dynamic> _parseResponse(http.Response response) {
    ...
    if (response.body.isNotEmpty) {
      try {
        ...
        if (decoded is Map<String, dynamic>) {
          ...
          throw ApiException('服务器返回的数据格式不正确', statusCode: response.statusCode);
        }
      } on FormatException {
    ...
    final message = rawError is String && rawError.isNotEmpty
        ? rawError
  ...
  void _debugResponse(http.Response response) {
    ...
    if (response.statusCode >= 400) {
      ...
      if (body.isNotEmpty) {
        ...
        final safeBody = body.length > maxLength
  ...
  _ImageUploadInfo _detectImageType(String filePath) {
    ...
    throw const ApiException('仅支持 JPG、PNG 或 WebP 图片');
  }
...
class _ImageUploadInfo {
  ...
  const _ImageUploadInfo({required this.contentType, required this.fileName});
}
