...
class RecommendationService
  ...
  static final RecommendationService
  instance =
  RecommendationService._();
  ...
  get unreadRecommendations =>
      List.unmodifiable(
        _unreadRecommendations,
      );

  List<ClothingRecommendation>
  get receivedRecommendations =>
      List.unmodifiable(
        _receivedRecommendations,
      );

  List<ClothingRecommendation>
  get sentRecommendations =>
      List.unmodifiable(
        _sentRecommendations,
      );
  ...
  get recommendations {
    ...
    for (final item
    in _receivedRecommendations) {
      map[item.id] =
          item;
    }

    for (final item
    in _sentRecommendations) {
      map[item.id] =
          item;
    }
  ...
  get latestUnreadRecommendation {
  ...
  /// 这个接口返回所有未读推荐，
  /// 最新推荐位于 index 0。
  Future<List<ClothingRecommendation>>
  refreshUnreadRecommendations() async {
    _requireLogin();
  ...
  /// 的完整收到推荐历史。
  Future<List<ClothingRecommendation>>
  refreshReceivedRecommendations() async {
    _requireLogin();
  ...
  Future<List<ClothingRecommendation>>
  refreshSentRecommendations() async {
    _requireLogin();
  ...
  markAsRead(
    ...
    try {
      ...
      /// 当前信从首页堆叠队列移除。
      _unreadRecommendations.removeWhere(
            (item) =>
        item.id ==
            recommendationId,
      );
  ...
  _parseRecommendation(
    ...
    try {
      return ClothingRecommendation.fromJson(
